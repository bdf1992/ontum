...
- [gradient.py](gradient.py) — the speed-gradient fold (done-line 0182, epic.graded-speed): the loop reading its own acts as one graded gradient of *speed* — the meta-level an act reaches, not its latency. bdo, 2026-06-21: *"multiple speeds … a gradient, not binary: thresholds binary, state graded."* Not a new control loop (that would double-build §14/D-12 — the fast↔medium bands already slide in code, `orchestrate` ↔ `slowloop`/`disposer`); a read-only *reading* over the shared log that names the band each act sits in, by **act-verb** (the naming reconciliation: bdo's "medium" is the doctrine's "slow loop", so bands are verbs not speeds): **respond** (fast — every event, receipt, `tick`), **retune** (medium — `setpoint`/`auto_admit_fence`), **author** (slow — `workflow_armed`/`node_real`/`surface`/`herald_introduction`/`tag`); a type it does not place is honestly **unbanded** (absence is information). The §10 tooth **untraced-band-crossing**: a `.claude/workflows/*.js` whose current bytes carry no enabled `workflow_armed` admission is a slow act the ledger cannot see — flagged with the one move, composing `review.is_armed`'s bytes-binding (no second arming definition, I-4; `tests/test_gradient.py` cross-checks against a real arm). Sibling of `gaps`/`census`/`heal` on the *speed* axis; read-only, propose-only (D-4). The dogfood is the point: on the live log all authored workflows read untraced — the `author` band is the disconnected island the arc exists to wire. The session record / attribute schema, the A4 run rail, and faithful `lint.py` are later pieces (graded-speed.proposal.md).
- [forest.py](forest.py) — the whole-tree viewport forest fold (done-line 0186, epic.whole-tree-viewport proposed): the loop sensing the whole *forest* of in-flight work, not just the landed trunk. bdo, 2026-06-22: *"I want my viewport to be the whole tree … a decorated/tagged/flagged instance of the work, in repo, managed automatically … I don't want MY viewport to be a blocker anymore."* Today the viewport is the primary checkout pinned to origin/main, so it shows only LANDED work — the forest (worktrees, branches, parked atoms, open PRs) is invisible and the checkout is load-bearing. A pure read-only fold in the `census`/`digest`/`gaps` grain (stdlib, deterministic, `--json`, a `done | report | needs-you` line) that COMPOSES, never rebuilds (§10): the git forest is sensed the same way `git.py garden`/`garden_verdict` sense it (`git worktree list --porcelain`, `gh pr list --state all`, per-worktree `git status --porcelain`, loose branches — git.py is a skill pen, not importable, so re-sensing via subprocess is the same sources, not a second truth), and atom state is `reconcile.Fold` + `orchestrate.next_action` + the digest's `atoms_on_main` (the D-13 join). Each work-item wears one closed-vocabulary status — **parked-atom** / **stranded** / **live-worktree** / **in-review** / **merged** — and the §10 teeth live in two pure classifiers: a *stranded* worktree (uncommitted work) can never be flagged `merged` even when its branch landed (the garden merged-but-dirty bite — the work wins), and a *parked* atom can never read `merged`/landed (`tests/test_forest.py` catches a constant classifier). Renders a regenerable `FOREST.md` (generated output, do-not-hand-edit; `--write` regenerates, default renders to stdout and writes nothing) + a `--json` twin. Read-only (I-3): it senses and names; it prunes, commits, and judges nothing — the cut stays bdo's (D-4). Live sessions/environments (`watcher.py` as a source, v1), the served HTML view, and the auto-regen hook/tick wiring are named-not-built (whole-tree-viewport.proposal.md).
- [session.py](session.py) — the session write-posture witness (done-line 0196, `session-read-write-mode.proposal.md`): a session's most basic posture — was it a **reader** or a **writer**, and when did it cross? — made a recorded fact at the cheapest grain. bdo, 2026-06-23: *"start every session in READ only mode; turning write on is a named event alongside the first actual write … when you read, you're good… for now."* Every session is born read-only; its first deliberate write is preceded by a `session_write_on` line naming the read → write crossing, so posture is never a stored flag — it is **folded from the log** (a reader writes no such record and never appears as a writer). The **session** axis the sibling `gradient.py` (the *act* axis) and `forest.py` (the *work-item* axis) do not carry; it mints no identity — it reuses the local Claude `session_id` (the `watcher.load_registry` reader, I-4) and the axis is `write_posture` (`reader | writer`), deliberately **not** `mode` (taken by `active_mode`'s security posture). The **`write-on` pen** is the one write path (I-2): it appends one record via `reconcile.append_line` carrying a required non-empty **narration** (the warm journalistic monologue a cold reader can pick up — justification · the way you'll write · authorship context; quality never machine-judged), an `emergency` flag, and `triggering_act`; it resolves the id from `CLAUDE_CODE_SESSION_ID` (`needs-you` if unset) and is **write-once** — a second declaration for a crossed session is a no-op (the `node.py` discipline). The **read-only fold** crosses the local session set against those records and reports per-session posture + the reader/writer census, **bounded** (one linear pass, the `trespass_count` grain — never quadratic). This is the **witness** increment — it **stops nothing and breaks nothing**; the consequence-fence under policy (the L1 `append_line` chokepoint + the L2 working-tree/log-diff observer, with the bypass red-team suite) is **increment 2**, named not built (witness before actuator). `tests/test_session.py` is the §10 tooth (a constant fold or a double-writing pen is caught). Read-only, stdlib only; the cut stays bdo's (D-4).
