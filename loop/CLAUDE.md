...
- [owner.py](owner.py) — the owner-gesture index (the owner-harness): the one place that answers *"bdo needs to decide X — what is the gesture surface?"* bdo authorizes by a gesture on a SURFACE (a GitHub issue he closes/comments in plain language), never a command; the openers exist but are scattered across five files (`arc`/`realness`/`rung`/`policy-intake` + `issue.py`), so a session that cannot find the surface falls back to a CLI line at bdo (the offload the hard rules forbid). A read-only fold (gaps/parity grain) mapping each owner-decision KIND to the one opener pen, surfaced by `--kind`; a decision with no GitHub surface yet (today `done-supersede`) is a named gap, never a CLI-at-owner fallback. The §10 teeth (`validate`, `tests/test_owner.py`, non-vacuous): a ghost opener that resolves to nothing is caught, a CLI-at-owner route (a `--by bdo` confirm dressed as an opener) is **refused** — the index can never legitimise handing bdo a command — and a mis-flagged gap is caught. Read-only, propose-grain; the cut and the missing surfaces stay bdo's (D-4).
