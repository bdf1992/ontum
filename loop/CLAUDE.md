...
- [session_gateway.py](session_gateway.py) — the session gateway, bind-at-birth (done-line 0195, issue #534, session-gateway.proposal.md §14). The first real chapter of the session gateway: the sibling of `loop/inference.py` at the session-birth seam — where the inference gateway authorizes a *thought*, this authorizes a *session*. Three pure parts. `derive_type(payload)` is the PURE, named, **versioned** typing function (a `TYPING_VERSION` constant the binding carries — proposal §10.1 / doctrine §14: a typing-rule change is new lineage, never a silent reinterpretation) mapping a session's birth signals to one closed role — **builder** (a worktree bench), **node-fill** (a branded `ontum-node:<id>` spawn), **steerer-admin** (the confirmed viewport) — with an honest least-privilege floor (an unconfirmed tree never gets steerer powers). Capability sets are **governed vocabulary** (the `loop/tags.py` shape): a closed core per type in code (`land` is in none by design — no default type lands its own line, D-2) PLUS admitted `session_capability` extensions. `bind()` writes ONE `session_binding` admission carrying the three A's (authenticated who/what · authorized capability set · attributed workspace + `--by` + `ts` + `typing_version`) and is **IDEMPOTENT** — an existing binding is REUSED, never blind-recreated (the rescue-branch-sprawl bug §4/§14 names). `emit_manifest()` writes the workstation portal(s): the branded tools (folded from `.claude/skills/*/SKILL.md` + `loop/*.py`) FILTERED by the type's capability set — **the PRESENCE of a tool's portal IS the fact that the gateway produced it and this type is authorized to use it** (bdo's portal model, 2026-06-23). The §10 tooth (`tests/test_session_gateway.py`, non-vacuous): a tool absent from a type's capability set is absent from its manifest, and a constant "all-authorized" classifier is caught; a binding is never blind-recreated. Read/propose-grain but for the two config pens; stdlib, local-first. New-tree provisioning, the SessionStart hook wiring, and the git-pen enforcement seam are named-not-built (the next increments, bdo's to stamp).
