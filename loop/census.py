#!/usr/bin/env python3
"""The organ census (done-line 0029): the loop sensing its own body.
...
This is its sibling for the repo's own organs — which modules, hooks,
...
stdlib — no subprocess, no network, no git (hard rule):
  ...
  wired      — some live source file (not the organ's own test) imports
               or invokes it. Reachability into the working system.
  exercised  — the organ's source speaks at least one *word the record
...
three movements:
  ...
  dormant     neither wired nor exercised — built, perhaps tested, but
              the metabolism never touches it. A prune candidate.

"exercised" is a vocabulary trace, not a call graph — it can miss an
organ that records only words it shares with no one, and the output
...
CLI:
  python -m loop.census          the organ census, read-only
"""
...
# Where organs live, as globs under the repo root. Filesystem-derived so
# a new organ enrolls itself — no hand-list to rot (the §12 trap).
ORGAN_GLOBS = (
    ("loop-module", "loop/*.py"),
...
# The append-only ledgers an organ's words can land in: the three logs,
...
# are what prove an organ's vocabulary actually flowed.
...
# An organ is a *writer* if its source can append to a record. A reader —
...
# use), not the tests (an organ used only by its own test is not wired).
...
def ref_regex(organ):
    """The pattern that means 'this organ is being used' — an import, a
    ...
    n = re.escape(organ["name"])
    pats = [re.escape(organ["rel"]), rf"\b{n}\.py\b"]
    if organ["kind"] == "loop-module":
        pats += [rf"\bloop\.{n}\b", rf"\bloop/{n}\b",
                 rf"from\s+loop\s+import\s+[^\n]*\b{n}\b",
                 rf"from\s+\.{n}\s+import", rf"from\s+\.\s+import\s+[^\n]*\b{n}\b"]
    elif organ["kind"] == "glyph":
...
def classify(organ, corpus, vocab, settings_text):
    src = ""
    try:
        src = organ["path"].read_text(encoding="utf-8", errors="ignore")
    except OSError:
        pass
    rx = ref_regex(organ)

    importers = sorted({rel for rel, text in corpus
                        if rel != organ["rel"] and rx.search(text)})
    ...
    return {**organ, "importers": importers, "entrypoint": entrypoint,
...
def render(results):
    ...
    print()
    if dormant or idle:
        print(f"result: report — {len(results)} organ(s): "
        ...
        print(f"result: done — all {len(results)} organ(s) alive; "
              f"every organ speaks a word the record holds")
    return 0
