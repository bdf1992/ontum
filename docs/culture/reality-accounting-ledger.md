...
phrase by phrase, into organs that already exist on disk, and the seams
...
- [loop/observe.py](../../loop/observe.py), [loop/relation_ledger.py](../../loop/relation_ledger.py), [loop/over_containment.py](../../loop/over_containment.py) — the consequence/observation organs *(implemented)*
...
next pressure.* Then fit one organ to write it.
...
journal cites (and the thesis organs), against what is on disk on `main`
...
| `structured-communication-channel.proposal.md` | [.ai-native/proposals/](../../.ai-native/proposals/structured-communication-channel.proposal.md) | the typed owner↔agent channel | **proposed** (its first organ shipped as `/recommend`) |
...
### Thesis organs — all resolve and participate
...
- The **organs and their grades** — the running code cited inline; the
