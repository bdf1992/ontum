...
claude -p "Run ONE overnight administer tick for the branded Claude surface. Read vscode/ontum-surface/ADMINISTER.md and follow it exactly: check the stop conditions FIRST (marker-met / STOPPED-infeasible / clock at-or-after 08:00), then do ONE bounded increment, verify it with a real check, update PROGRESS.md and PARITY-CHECKLIST.md, commit through the git pen (.claude/skills/branch-ritual/git.py — never raw git add/commit), and push the branch. If the marker is met, run the self-grade + /code-review ultra peer grade + open the PR with GRADES.md per ADMINISTER.md, then set STATE so later ticks exit." --model %MODEL% --permission-mode bypassPermissions --append-system-prompt "You are an UNATTENDED overnight tick. Be conservative: exactly one increment, honest checkable evidence, commit, then stop. Never push to main, never merge, never bypass the command_guard fence. Under-claim before you over-claim." >> %LOG% 2>&1
