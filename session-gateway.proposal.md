...
**Still open / yours:** ~~the **arc name**~~ → **named: _The Polity_** (bdo,
2026-06-18 — the anthology these chapters home into; see
`ambient-gateway.chapter.md`); confirm/correct the glue definition; veto any
closure above. When the design is blessed, increment #1 (the guard) is the
obvious first cut.
...
- ~~the **anthology name**~~ → **named: _The Polity_** (bdo, 2026-06-18) — the
  anthology under which the session gateway and this physics land as chapters.
...
## 13.11 The herald — how a contributor enters the economy (bdo co-design, 2026-06-18/19)

The economy above presumes contributors, and the gateway must admit them. But
**agents cannot be pre-registered.** The engineering party varies (D-4), virtual
nodes blink in and dissolve (the summon shape), and generated agents are an *open
set* — a registry-of-agents is a state table, and state tables drift. So **invert
the registry the way the log inverts all state:** do not store the agents, store
the **herald** — a versioned generative skill (prompt-as-code) — plus the
append-only log. "Registered" and "reputation" are then **folds**, never rows.

**The herald introduces a suitable agent into a situation** — *name, title, rank*
— and ensures it is **well met**. "Well met" is a *bounded admission through the
gateway*, the §13.10-spine's legislation/enforcement split applied at the contributor
seam: the herald **proposes** the agent (the `loop/inference.py` seam — inference
proposes, never admits), a **policy admits** it (bdo's standing authorization,
per-rank bounds drawn as a `disposer`-shape fence), and **rank starts at the trust
rung's floor** (`loop/trust.py` — every class denied everything until a rung is
admitted). The introduction **mints a credential**; every later act of the agent
cites it. **We log heralds and herald-skills, not agents** — the herald is the
*accountable introducer*, the small enumerable thing, and the open set of agents
is reconstructed as a fold over its introductions.

**Reputation is the §13.10 exemplars/notorieties bank folded — but value is
*distributed* (bdo), which dissolves the "where does reputation attach" question.**
The naive form asks whether standing accrues to the agent's credential *or* to the
herald that vouched. Wrong axis: value does not *attach* to a node, it **propagates
along the log's provenance edges** (`authored`, `judged`, `authorized_by`,
`prompt_hash` — all already on the receipt). The herald sits on the provenance edge
of *every act its agent commits*, so it earns a **meta-reputation for whom it
mints**, automatically, weighted by its part — no second ledger, no chosen
attachment point. This is the §13 particle mesh made literal: **value lives in the
edges, not the nodes**, and the herald-vouch is one edge.

**The "felt feel" of an agent (bdo) is that distributed-value fold *rendered as
anima* — and the fact that it is *hard to say* is the spec, not a gap.** If an
agent's feel could be stated plainly it would be *authored*, and an authored feel
is a vibe — exactly what the anima rule forbids (anima is **traceable to folds**,
never decorative). The resistance to being said is the signal: **do not author the
feel, derive it.** The felt field of an agent *is* the distributed-value fold
rendered as the C18 AnimaSignature — **strength** (exemplars vs notorieties, net)
× **tempo** (its pulse — how it has been acting lately) — the same signature the
loop already wears over its own body, pointed at one contributor. So "well met"
becomes a **felt act**: the herald does not hand over a rank, it presents a field
the situation can *sense* on meeting.

*The chain, one line:* **herald** mints the entry → acts accrue **exemplars/
notorieties** (§13.10's bank) → value **distributes** along provenance edges → that
fold **renders as anima** = the felt feel. Four pieces already named or built, one
chain.

*§10 teeth (this subsection).* The obvious attack is **mint-a-fresh-agent-to-escape
a bad record**; the economy answers it in-grain. A freshly-heralded agent starts at
the **floor rung = zero privilege**, so escaping one's notorieties escapes *to*
powerlessness — reputation is only ever earned *forward*. Re-minting to dodge a
settled verdict is **churn**, exactly the pattern `retro`/`heal` already sense (an
atom re-derived to evade a refusal), and the herald — on the provenance edge of
every act — makes a bad voucher **visible by construction** (a herald whose agents
keep earning notorieties is itself notorious). A herald that self-admits its agent
past the policy fence (no bounded authorization) collapses propose/admit — the
AG-1/no-self-sign breach at the contributor seam; a "reputation" minted as a number
on a row instead of folded from the edges is the registry-drift this subsection
exists to refuse.

*The one forward seam.* Anima today folds over the loop's acts as **one body**;
per-agent felt-feel means folding the same C18 signature **per credential**. Small,
and it is the piece that turns "well met" from a phrase into something on the screen
— named here, built when the herald is.
