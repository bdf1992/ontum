...
await agent(
  `Write the authored tender to \`.claude/workflows/${filename}\` (a new file; the workflows CLAUDE.md governs it). ` +
  'The content is base64 below — decode it and write the file verbatim, then report the byte count and that the file parses ' +
  `(run \`node --check .claude/workflows/${filename}\`).\n` +
  `Run from the repo root:\n` +
  `python -c "import base64,pathlib; pathlib.Path('.claude/workflows/${filename}').write_bytes(base64.b64decode('${b64}'))"`,
